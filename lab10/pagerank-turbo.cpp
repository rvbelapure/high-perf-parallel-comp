...
double* page_rank_iteration_optimized(double *CSE6230_RESTRICT probabilities_new, const double *CSE6230_RESTRICT probabilities_old,
	...
	const int32_t *CSE6230_RESTRICT link_free_pages, int32_t pages_count, int32_t link_free_pages_count)
{
	...
	for (int32_t link_free_page_index = 0; link_free_page_index < link_free_pages_count; link_free_page_index++) {
		...
		link_free_prob += probabilities_old[column_index];
	}
	link_free_prob /= double(pages_count);

	for (int32_t page = 0; page < pages_count; page++) {
		double transition_probability = link_free_prob;
